...
export function StaffProfileView({ user }: { user: any }) {
  return (
      ...
      <div className="flex flex-col md:flex-row gap-6 px-4 -mt-16 relative z-10">
        <div className="flex flex-col items-center md:items-start gap-4">
            ...
            <div className="text-center md:text-left space-y-1">
                <h1 className="text-2xl font-bold">{user?.displayName || "Staff Member"}</h1>
                <p className="text-muted-foreground font-medium flex items-center justify-center md:justify-start gap-2 text-sm">
                    <Briefcase className="h-3.5 w-3.5" /> Senior Professor, Dept. of Physics
                </p>
                <div className="flex flex-wrap gap-2 pt-2 justify-center md:justify-start">
                    <Badge className="bg-blue-100 text-blue-700 border-none px-3 py-1">Permanent Faculty</Badge>
                    <Badge className="bg-green-100 text-green-700 border-none px-3 py-1">Active</Badge>
                </div>
            </div>
        </div>
        ...
        <div className="flex-1 grid gap-6 md:grid-cols-2 lg:grid-cols-3 mt-16 md:mt-16">
            ...
            <Card className="bg-card/40 backdrop-blur-sm border-none shadow-sm md:col-span-2 lg:col-span-1">
                ...
                <CardContent>
                    <p className="font-medium">Block B, Room 402, Science Wing</p>
                </CardContent>
            </Card>
        </div>
      </div>
        ...
        <Card className="bg-card/30 border-none shadow-sm">
            ...
            <CardContent className="space-y-4">
                <div className="space-y-1">
                    ...
                    <p className="text-sm font-semibold leading-relaxed">Ph.D. in Quantum Mechanics from University of Excellence</p>
                </div>
                <div className="space-y-1">
                    <p className="text-xs text-muted-foreground uppercase font-bold tracking-wider">Experience</p>
                    <p className="text-sm font-semibold led">12+ Years in Research and Teaching</p>
                </div>
                <div className="space-y-1">
                    <p className="text-xs text-muted-foreground uppercase font-bold tracking-wider">Specialization</p>
                    <div className="flex flex-wrap gap-2 pt-1">
                        {["Numerical Analysis", "Astrophysics", "Particle Physics"].map((s, i) => (
        ...
        <Card className="bg-card/30 border-none shadow-sm">
            ...
            <CardContent className="space-y-4">
                <div className="flex justify-between items-center py-1.5 border-b border-muted/50">
                    ...
                    <span className="text-sm font-bold">Aug 15, 2018</span>
                </div>
                <div className="flex justify-between items-center py-1.5 border-b border-muted/50">
                    <span className="text-sm text-muted-foreground flex items-center gap-2">
                        <Briefcase className="h-4 w-4" /> Staff ID
                    </span>
                    <span className="text-sm font-bold font-mono">STF-2018-092</span>
                </div>
                <div className="flex justify-between items-center py-1.5 border-b border-muted/50">
                    <span className="text-sm text-muted-foreground flex items-center gap-2">
                        <ShieldCheck className="h-4 w-4" /> Role Access
                    </span>
                    <span className="text-sm font-bold">Faculty Admin</span>
                </div>
            </CardContent>
        </Card>
      </div>
